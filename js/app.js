...
function playSeries(series) {
    ...
    series.timer = null;
    series.mode = 'replay';
    if (series.currentIndex >= series.records.length - 1) {
        series.currentIndex = 0;
    }
    series.isPlaying = true;
    scheduleSeriesTick(series);
...
function scheduleSeriesTick(series) {
    ...
    const delayMs = nextIndex === 0
        ? Math.max(300, firstStep)
        : Math.max(300, next.timestampMs - current.timestampMs);
...
function bindChartControls() {
    els.sensorList.querySelectorAll('.chart-control-panel button[data-action]').forEach(button => {
        button.dataset.controlBound = 'true';
        ['click', 'pointerup', 'mouseup'].forEach(eventName => {
            button.addEventListener(eventName, () => handleChartControlActionOnce(button));
        });
    });
    els.sensorList.querySelectorAll('.chart-control-panel input[data-action]').forEach(input => {
        input.dataset.controlBound = 'true';
        input.addEventListener('input', () => handleChartControlInputElement(input));
    });
}
...
function setupEventListeners() {
    ...
    document.addEventListener('pointerdown', handleChartControlClick, true);
    document.addEventListener('mousedown', handleChartControlClick, true);
    document.addEventListener('click', handleChartControlClick, true);
    document.addEventListener('pointerup', handleChartControlClick, true);
    document.addEventListener('mouseup', handleChartControlClick, true);
    document.addEventListener('input', handleChartControlInput, true);
...
function handleChartControlClick(event) {
    ...
    if (!control) return;
    handleChartControlActionOnce(control);
}

function handleChartControlActionOnce(control) {
    const now = Date.now();
    const lastActionAt = Number(control.dataset.lastActionAt || 0);
    if (now - lastActionAt < 200) return;
    control.dataset.lastActionAt = String(now);
    handleChartControlAction(control);
}

function handleChartControlAction(control) {
    const series = seriesFromControl(control);
    if (!series) {
        console.error('Replay control cannot find sensor series.', {
            action: control.dataset.action,
            category: control.closest('.chart-control-panel')?.dataset.category,
            sensorName: control.closest('.chart-control-panel')?.dataset.sensorName
        });
        return;
    }
    ...
    console.log('Replay control action:', {
        action,
        mode: control.dataset.mode || null,
...
function handleChartControlInput(event) {
    ...
    if (!control) return;
    handleChartControlInputElement(control);
}

function handleChartControlInputElement(control) {
    const series = seriesFromControl(control);
    if (!series) return;
...
window.SensorDashboard = {
    ...
    renderEmptyState,
    escapeHtml,
    handleChartControlActionOnce,
    handleChartControlAction
};
